...
int
bnxt_hwrm_ring_alloc(bnxt_ring_t *rp, bnxt_ring_type_t type)
{
	bnxt_t *bp = rp->bp;
	...
	switch (type) {
	case BNXT_RING_CP:
		req.ring_type = HWRM_RING_ALLOC_INPUT_RING_TYPE_L2_CMPL;
...
int
bnxt_hwrm_ring_free(bnxt_ring_t *rp, bnxt_ring_type_t type)
{
	bnxt_t *bp = rp->bp;
	...
	switch (type) {
	case BNXT_RING_CP:
		req.ring_type = HWRM_RING_FREE_INPUT_RING_TYPE_L2_CMPL;
		req.cmpl_ring = (uint16_t)HWRM_NA_SIGNATURE;
		phys_id = &rp->cp_phys_id;
		break;
	case BNXT_RING_RX:
		req.ring_type = HWRM_RING_FREE_INPUT_RING_TYPE_RX;
		req.cmpl_ring = rp->cp_phys_id;
		break;
	case BNXT_RING_TX:
		req.ring_type = HWRM_RING_FREE_INPUT_RING_TYPE_TX;
		req.cmpl_ring = rp->cp_phys_id;
		break;
	}
